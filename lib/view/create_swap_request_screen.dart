...
class _CreateSwapRequestScreenState extends State<CreateSwapRequestScreen> {
  ...
  final MySkillsRepository _mySkillsRepository = MySkillsRepository.instance;
  ...
  Color get _highlightBackground => _isDarkMode
      ? _primaryColor.withValues(alpha: 0.14)
      : const Color(0xFFE4F0EF);

  Color get _highlightBorder => _isDarkMode
      ? _primaryColor.withValues(alpha: 0.28)
      : const Color(0xFFD2E5E2);

  List<String> get _supportedModes {
    final Skill? learnSkill = _resolvedSkillToLearn;
    final Skill? offerSkill = _selectedSkillToOffer;

    if (learnSkill == null || offerSkill == null) {
      return const <String>[];
    }

    final List<String> modes = <String>[];

    if (learnSkill.supportsSessionMode('Online') &&
        offerSkill.supportsSessionMode('Online')) {
      modes.add('Online');
    }

    if (learnSkill.supportsSessionMode('In-person') &&
        offerSkill.supportsSessionMode('In-person')) {
      modes.add('In-person');
    }
  ...
  bool get _hasCommonSessionMode => _supportedModes.isNotEmpty;
  ...
  Future<void> _loadScreenData() async {
    ...
    try {
      final String currentUserId = _currentUserService.userId.trim();
      ...
      await _mySkillsRepository.getOfferedSkills(currentUserId);

      if (!mounted) {
        return;
      }

      final String? learnSkillId = _resolvedSkillToLearn?.id;

      final List<Skill> availableSkills = managedSkills
          .map((ManagedSkill item) => item.skill)
          .where((Skill skill) => skill.id != learnSkillId)
          .toList();
      ...
      final Skill? preselectedSkill = _resolveBestOfferSkill(availableSkills);

      setState(() {
        _mySkills
          ..clear()
          ..addAll(availableSkills);

        if (_selectedSkillToOffer != null &&
            !_mySkills.any(
                  (Skill skill) => skill.id == _selectedSkillToOffer!.id,
            )) {
          _selectedSkillToOffer = null;
        }

        _selectedSkillToOffer ??= preselectedSkill;

        _normalizeSelectedMode(clearMeetingDetails: false);
  ...
  Skill? _resolveBestOfferSkill(List<Skill> availableSkills) {
    if (availableSkills.isEmpty) {
      return null;
    }

    final String? preferredId = _cleanOptionalId(widget.skillToOfferId);
    ...
    final String preferredTitle =
        widget.skillToOffer?.trim().toLowerCase() ?? '';

    if (preferredTitle.isNotEmpty) {
      final List<Skill> titleMatches = availableSkills
          .where(
            (Skill skill) =>
        skill.title.trim().toLowerCase() == preferredTitle,
      )
          .toList();
    ...
    final User? provider = _resolvedProvider;

    if (provider == null) {
      return null;
    }

    final Set<String> providerWantedSkillIds = _exploreRepository
        .getWantedSkillsForUser(provider.id)
    ...
    final List<Skill> reciprocalMatches = availableSkills
        .where((Skill skill) => providerWantedSkillIds.contains(skill.id))
        .toList();
  ...
  bool _isModeSupported(String mode) => _supportedModes.contains(mode);

  void _normalizeSelectedMode({required bool clearMeetingDetails}) {
    final List<String> modes = _supportedModes;
    final String? current = _selectedMode;

    if (current != null && modes.contains(current)) {
      return;
    }

    _selectedMode = modes.isEmpty ? null : modes.first;
  ...
  void _selectMode(String mode) {
    if (_isSending || !_isModeSupported(mode) || _selectedMode == mode) {
      return;
    }
  ...
  void _markChanged() {
    if (_hasUnsavedChanges || _isLoading || _isSending) {
      return;
    }
  ...
  Future<void> _handleBackPressed() async {
    if (_isSending || _isShowingLeaveDialog) {
      return;
    }
    ...
    final bool shouldDiscard = await _showDiscardConfirmation();
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style: AppTextStyles.cardTitle.copyWith(color: _textColor),
                ),
              ),
            ],
          ),
  ...
  // ============================================================
  // RESOLVED PROVIDER / SKILL
  // ============================================================

  User? get _resolvedProvider {
    final String? suppliedId = _cleanOptionalId(widget.providerUserId);

    if (suppliedId != null) {
      return _exploreRepository.findUserById(suppliedId);
    }

    final String targetName = widget.providerName.trim().toLowerCase();

    if (targetName.isEmpty) {
      return null;
    }

    final List<User> matches = _exploreRepository.users
        .where(
          (User user) => user.name.trim().toLowerCase() == targetName,
    )
        .toList();
  ...
  String? get _resolvedProviderUserId => _resolvedProvider?.id;

  Skill? get _resolvedSkillToLearn {
    final String? suppliedId = _cleanOptionalId(widget.skillToLearnId);

    if (suppliedId != null) {
      return _exploreRepository.findSkillById(suppliedId);
    }

    final String targetTitle = widget.skillToLearn.trim().toLowerCase();

    if (targetTitle.isEmpty) {
      return null;
    }

    final List<Skill> matches = _exploreRepository.skills
        .where(
          (Skill skill) => skill.title.trim().toLowerCase() == targetTitle,
    )
        .toList();
  ...
  bool get _providerStillOffersLearnSkill {
    final User? provider = _resolvedProvider;
    final Skill? skill = _resolvedSkillToLearn;
    ...
    return _exploreRepository
        .getOfferedSkillsForUser(provider.id)
        .any((relationship) => relationship.skillId == skill.id);
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: !_isSending && !_hasUnsavedChanges,
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          leading: IconButton(
            onPressed: _isSending ? null : _handleBackPressed,
            icon: Icon(
              Icons.arrow_back_ios_new_rounded,
              size: 19,
              color: _isSending ? _mutedColor : _primaryColor,
            ),
          ),
          title: Text(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Loading your skills...',
              style: AppTextStyles.secondary.copyWith(color: _mutedColor),
            ),
          ],
        ),
      );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSectionLabel('You want to learn'),
          const SizedBox(height: 8),
          _buildReadOnlySkill(),
          const SizedBox(height: 22),
          _buildSectionLabel('What can you offer?'),
          ...
          _buildSkillDropdown(),
          const SizedBox(height: 22),
          _buildSectionLabel('Preferred schedule'),
          const SizedBox(height: 5),
          Text(
          ...
          const SizedBox(height: 22),
          _buildSectionLabel('Session mode'),
          const SizedBox(height: 5),
          Text(
          ...
          const SizedBox(height: 10),
          _buildModeSelector(),
          if (_selectedSkillToOffer != null && !_hasCommonSessionMode) ...[
            const SizedBox(height: 10),
          ...
          TextField(
            ...
            style: AppTextStyles.input.copyWith(color: _textColor),
            decoration: InputDecoration(
              hintText:
              'Example: Hi! I would love to learn this skill. I can help you with...',
              hintStyle: AppTextStyles.inputHint.copyWith(color: _mutedColor),
              filled: true,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: _canAttemptSend ? _sendRequest : null,
              child: _isSending
                  ? Row(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  skill.mode,
                  style: AppTextStyles.caption.copyWith(color: _mutedColor),
                ),
              ],
            ),
          ),
  ...
  Widget _buildSkillDropdown() {
    ...
    return DropdownButtonFormField<Skill>(
      key: ValueKey<String?>(_selectedSkillToOffer?.id),
      initialValue: _selectedSkillToOffer,
      ...
      decoration: InputDecoration(
        hintText: 'Select a skill you can teach',
        hintStyle: AppTextStyles.inputHint.copyWith(color: _mutedColor),
        filled: true,
        fillColor: _surfaceColor,
      ),
      items: _mySkills.map((Skill skill) {
        // Keep the selected field and popup item to one visual line.
        // The previous two-line Column could overflow the dense
        // DropdownButtonFormField height on real Android devices.
      ...
      onChanged: _isSending
          ...
          : (Skill? value) {
        setState(() {
          final String? oldMode = _selectedMode;

          _selectedSkillToOffer = value;
          _normalizeSelectedMode(clearMeetingDetails: false);

          if (oldMode != _selectedMode) {
            _meetingDetailsController.clear();
          }
            ...
            Expanded(
              child: Text(
                ...
                style: _selectedDate == null
                    ? AppTextStyles.inputHint.copyWith(color: _mutedColor)
                    : AppTextStyles.input.copyWith(color: _textColor),
              ),
            ),
          ],
        ),
  ...
  Future<void> _selectDate() async {
    ...
    if (result == null || !mounted || _isSending) {
      return;
    }
  ...
  Widget _buildModeOption({
    ...
    final bool supported = _isModeSupported(label);
    final bool selected = supported && _selectedMode == label;
    final bool enabled = supported && !_isSending;
    ...
    return Opacity(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: selected
                ? _primaryColor.withValues(alpha: _isDarkMode ? 0.18 : 0.10)
                : _surfaceColor,
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
  ...
  Widget _buildMeetingDetails() {
    final String? selectedMode = _selectedMode;
    ...
    final bool online = selectedMode == 'Online';

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          online ? 'Preferred online platform' : 'Preferred meeting area',
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            hintStyle: AppTextStyles.inputHint.copyWith(color: _mutedColor),
            prefixIcon: Icon(
              online ? Icons.language_rounded : Icons.location_on_outlined,
              size: 20,
              color: _primaryColor,
            ),
            filled: true,
  ...
  Widget _buildDisabledMeetingDetails(String message) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryRow('Learn', _resolvedSkillToLearn!.title),
          _buildSummaryRow(
            'Offer',
            _selectedSkillToOffer?.title ?? 'Not selected',
          ),
          _buildSummaryRow(
            'Schedule',
  ...
  Future<void> _sendRequest() async {
    ...
    final String requesterUserId = _currentUserService.userId.trim();
    final User? provider = _resolvedProvider;
    final String? providerUserId = _resolvedProviderUserId;
    final Skill? skillToLearn = _resolvedSkillToLearn;
    final Skill? skillToOffer = _selectedSkillToOffer;
    final String? selectedMode = _selectedMode;
    ...
    if (requesterUserId == providerUserId) {
      _showError('You cannot send a swap request to yourself.');
      return;
    }
    ...
    if (!_providerStillOffersLearnSkill) {
      _showError('This provider no longer offers the selected skill.');
      return;
    }
    ...
    if (skillToOffer == null) {
      if (_mySkills.isEmpty) {
        ...
        _showError('Please select a skill you can offer.');
      }
      return;
    }

    final bool stillOwnsOfferedSkill =
    _mySkills.any((Skill skill) => skill.id == skillToOffer.id);
    ...
    if (skillToLearn.id == skillToOffer.id) {
      _showError(
    ...
    if (selectedMode == null ||
        !_isModeSupported(selectedMode) ||
        !skillToLearn.supportsSessionMode(selectedMode) ||
        !skillToOffer.supportsSessionMode(selectedMode)) {
      _showError(
    ...
    if (_selectedDate == null || _selectedTime == null) {
      _showError('Please choose your preferred date and time.');
      return;
    }

    final DateTime proposedAt = DateTime(
      _selectedDate!.year,
      _selectedDate!.month,
      _selectedDate!.day,
      _selectedTime!.hour,
    ...
    final String meetingDetails = _meetingDetailsController.text.trim();
    ...
    if (meetingDetails.length > 150) {
      _showError('Meeting details must be 150 characters or less.');
      return;
    }

    final String cleanNote = _noteController.text.trim();

    if (cleanNote.length > 300) {
      _showError('Message must be 300 characters or less.');
      return;
    }
    ...
    try {
      await _swapService.createRequest(
        ...
        skillToLearnId: skillToLearn.id,
        skillToOfferId: skillToOffer.id,
        providerName: provider.name,
        providerInitials: provider.initials,
        providerCity: provider.city,
        skillToLearn: skillToLearn.title,
        skillToOffer: skillToOffer.title,
  ...
  String? _cleanOptionalId(String? value) {
    if (value == null) {
      return null;
    }
